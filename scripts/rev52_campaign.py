"""R52 supervisor: the cap-lifted control on block B of the span ladder.

Generated from rev51_campaign.py, which ran the same control on block A, with
the registry, stratum and design key switched and the stage priors replaced by
what block A actually cost: op 11, base 436, cal 8 and ladders of 45 minutes.
Block B is the same design at the same four levels, so those measurements are
the right prior for it. Everything else, including the deadline arithmetic and
the refusal to quote anything from an unfinished base, is unchanged.
...
Usage:
    python rev52_campaign.py --stop-at "2026-08-13 12:00" --workers 11
    python rev52_campaign.py --status
"""
...
REG = "r52"
STRATUM = "span_ladder_b_uncapped"
KEY = "RS2U"
BETAS = [1.00, 0.75, 0.62, 0.50]

LOG = HERE / "r52_campaign.log"
PROGRESS = METRICS / "r52_campaign_progress.json"

# measured on block A of this same ladder (R51): op 10.8, base 436.3, cal 8.3,
# ladders 44-46. The priors carry a margin over those so that a stage is never
# started with less time than it took last night.
OP_MIN = 13.0
BASE_MIN = 445.0
CAL_MIN = 12.0
LADDER_PRIOR_MIN = 50.0
RESERVE_MIN = 15.0
...
def main() -> int:
    ...
    py = sys.executable
    log(f"=== R52 cap-lifted span-ladder control, block B: stop at "
