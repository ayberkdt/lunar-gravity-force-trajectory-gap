"""Supervisor for R62 (O54): walk the registered ladder cells in one window.
...
Usage:
    python rev62_campaign.py --stop-at 2026-08-19T18:00 --workers 10
"""
...
PREREG = METRICS / "r62_preregistration.json"
PROGRESS = METRICS / "r62_campaign_progress.json"
DRIVER = HERE / "rev62_ladder_interior_rematch.py"

DISK_FLOOR_GB = 6.0
MIN_RECORD_BYTES = 1000
EXPECTED_ORBITS = 32          # 16 identities at each of the two levels
...
def record_path(key: str, beta: float) -> Path:
    return METRICS / f"r62_ladder_interior_{key}_{beta_tag(beta)}.json"
...
def main() -> int:
    ...
    state = {"schema": "r62_campaign_progress_v1",
    ...
    for i, cell in enumerate(cells, 1):
        ...
        if cell_done(key, beta):
            print(f"[r62] {label}: already complete, skipping", flush=True)
        ...
        if now >= stop_at or now + projected > stop_at:
            ...
            print(f"[r62] {left:.0f} min left, slowest cell so far "
        ...
        gb = free_gb(METRICS)
        if gb < DISK_FLOOR_GB:
            print(f"[r62] ABORT before {label}: {gb:.1f} GB free, floor is "
        ...
        print(f"=== [{i}/{len(cells)}] R62 {label} start "
        ...
        print(f"=== R62 {label} {status} rc={rc} {wall / 60:.1f} min ===",
              flush=True)
    ...
    print(f"[r62] window over: {n} cells completed this run", flush=True)
    return 0
