"""SHA-256 integrity manifest for R51: the cap-lifted span-ladder control.

R51 is not a new population. It is block A of the R50 paired ladder propagated
...
it as one: R50's registration, its frozen design for block A, its prepass rows
...
Usage:  python rev51_finalize_manifest.py
"""
...
KEY = "RS1U"
PARENT_REG, PARENT_NAME, PARENT_KEY = "r50", "span_ladder_a", "RS1"
K_INTERIOR = ("0.25", "0.50", "0.75")

SCRIPTS = ["rev51_uncapped_freeze.py", "rev51_campaign.py",
           "rev51_ceiling_probe.py", "rev51_finalize_manifest.py",
...
SCOPE = ("R51: block A of the R50 paired radial-span ladder re-propagated with "
         ...
         "which aligns it with that axis.")
...
def main() -> int:
    ...
    result_names = [f"{REG}_{NAME}_rows.json", f"{REG}_{NAME}_convergence.json",
                    ...
                    f"{REG}_verdict.json",
                    f"{REG}_capped_vs_uncapped_table.tex",
                    f"{REG}_capped_vs_uncapped_macros.tex",
                    ...
                    f"{REG}_ceiling_probe_registered_grid.json",
                    f"{REG}_ceiling_probe_all_budgets.json"]
    tree_map = {
    ...
    payload = {
        ...
        "registration_status": (
            ...
            "r51_preregistration.json before the first trajectory. The "
            "sufficiency of degree 600 was established before propagation by "
            "the demand probe recorded in the registration, and the freezing "
            "script refuses to register this control at all if that probe "
            "reports either that 600 fails to clear the demand or that the "
            "ceiling never binds."),
        "population": NAME,
        "design_key": KEY,
        "derived_from": {
