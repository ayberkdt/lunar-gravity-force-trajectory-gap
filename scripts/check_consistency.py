...
def check_manifest_counts(texts, problems, notes):
    ...
    for name, s in texts.items():
        ...
        # [\w-] rather than \w: NUMBER_WORDS defines the hyphenated forms, and
        # \w+ silently captured only the unit word, so "twenty-two" read as 2
        # and the check reported a mismatch against every count above twenty.
